...
from config import AWS_REGION, BEDROCK_MODEL_ID, BEDROCK_PUBLIC_ACCESS
...
def _fallback(rule_hits, url_reasons, *, reason="local_signals"):
    ...
    if "OTP_REQUEST" in ids or "PIN_REQUEST" in ids:
        ...
        cat = "Other/Unknown"
    summary = "The message was checked using local security signals."
    if reason == "public_access_disabled":
        summary = "The message was checked using local security signals. AI explanation is available after sign-in."
    return {
        "available": False,
        "summary": summary,
        "reasons": reasons,
        "recommended_action": "Do not share your OTP, PIN, password, or card details. If unsure, contact your bank using its official app, website, or phone number.",
        "scam_category": cat,
        "ai_risk_score": 0.0,
...
def analyze(sanitized_text, ml_score, url_score, rule_hits, language, url_reasons, *, allow_bedrock=True):
    if not BEDROCK_MODEL_ID:
        return _fallback(rule_hits, url_reasons, reason="model_not_configured")
    if not allow_bedrock and not BEDROCK_PUBLIC_ACCESS:
        return _fallback(rule_hits, url_reasons, reason="public_access_disabled")

    payload = {
        "sanitized_text": sanitized_text,
        "ml_score": ml_score,
        "url_score": url_score,
        "triggered_rules": [h["rule_id"] for h in rule_hits],
    ...
    try:
        ...
        response = client.converse(
            ...
            messages=[{
                "role": "user",
                "content": [{"text": "APPLICATION_METADATA_AND_USER_CONTENT:\n" + json.dumps(payload, ensure_ascii=False)}],
            }],
        ...
        return {
            ...
            "reasons": [str(x)[:180] for x in reasons[:6]],
            "recommended_action": str(data.get("recommended_action", ""))[:700],
            "scam_category": category,
            "ai_risk_score": max(0.0, min(100.0, score)),
        }
        ...
        return _fallback(rule_hits, url_reasons, reason="service_failure")
