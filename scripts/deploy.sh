...
COGNITO_CLIENT_ID="$(stack_output CognitoUserPoolClientId)"
COGNITO_REGION="$(stack_output CognitoRegion)"
...
python - "$API_URL" "$COGNITO_REGION" "$COGNITO_CLIENT_ID" "$DEPLOY_DIR/runtime-config.js" <<'PY'
import json
import pathlib
import sys
api_url, cognito_region, cognito_client_id, output = sys.argv[1:]
config = (
    "window.RURALSHIELD_CONFIG = {"
    f"apiUrl: {json.dumps(api_url)}, "
    f"cognitoRegion: {json.dumps(cognito_region)}, "
    f"cognitoClientId: {json.dumps(cognito_client_id)}"
    "};\n"
)
pathlib.Path(output).write_text(config, encoding="utf-8")
PY

aws s3 sync "$DEPLOY_DIR/" "s3://$FRONTEND_BUCKET/" --delete --region "$REGION"
aws cloudfront create-invalidation --distribution-id "$DISTRIBUTION_ID" --paths '/*' >/dev/null

printf '\nRuralShield AI deployed.\nAPI: %s\nFrontend (HTTPS): %s\nCognito client: %s\n' "$API_URL" "$FRONTEND_URL" "$COGNITO_CLIENT_ID"
